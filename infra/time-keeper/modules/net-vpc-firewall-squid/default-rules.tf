...
resource "google_compute_firewall" "allow-admins-squid" {
  count         = length(local.default_rules.admin_ranges) > 0 ? 1 : 0
  name          = "${var.network}-ingress-admins-${random_id.firewall.hex}"
...
resource "google_compute_firewall" "allow-tag-http-squid" {
  count         = length(local.default_rules.http_ranges) > 0 ? 1 : 0
  name          = "${var.network}-ingress-tag-http-${random_id.firewall.hex}"
...
resource "google_compute_firewall" "allow-tag-https-squid" {
  count         = length(local.default_rules.https_ranges) > 0 ? 1 : 0
  name          = "${var.network}-ingress-tag-https-${random_id.firewall.hex}"
  description   = "Allow http to machines with matching tags."
  network       = var.network
  project       = var.project_id
...
resource "google_compute_firewall" "allow-tag-ssh-squid" {
  count         = length(local.default_rules.ssh_ranges) > 0 ? 1 : 0
  name          = "${var.network}-ingress-tag-ssh-${random_id.firewall.hex}"
